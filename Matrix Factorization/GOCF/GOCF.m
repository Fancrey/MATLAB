function [U_final, V_final, nIter_final, objhistory_final] = GOCF(X, k, W, options, U, V)
...
% W ... weight matrix of the affinity graph %%其实是Sij
%
...
if ~isfield(options,'alpha')                                                %应该是论文中的lamda 2020.3.24
    options.alpha = 100;
end
...
switch lower(options.Optimization)
    case {lower('Multiplicative')} 
        [U_final, V_final, nIter_final,objhistory_final] = GOCF_Multi(K, k, W, options, U, V);
    otherwise
