    function [  U_final, V_final, nIter_final, objhistory_final] = GOCF_Multi(K, k, W, options, U, V)
...
% W ... weight matrix of the affinity graph 就是论文里的S
%
...
alpha = options.alpha;
miu   = options.miu; %2020.3.24 第一次修改
Norm = 2;
NormV = 1;
...
while tryNo < nRepeat
    ...
    while(maxErr > differror)
        ...
        if alpha > 0
            ...
            KU = KU + WV;
            VUKU = VUKU + alpha*DV; %修改 加了lamda 看原文中没有
        end
        ...
        %U = U.*(KV./max(KUVV,1e-10));  %原始版本 2020.3.24修改
        U = U.*((KV+2*miu*U)./max((KUVV+miu*2*U*U'*U),1e-10));  
        clear KV VV KUVV;
